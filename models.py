...
def Calcul_Mean(df: pd.DataFrame, weights) -> np.float64:
...
def Calcul_VaR_95 (df, weights) -> np.float64:
    returns = df.pct_change()
    new = np.sum(returns * weights)
    return new.quantile(0.05)


def Calcul_CVaR_95 (df, weights, VaR95) -> np.float64:
    returns = df.pct_change()
    new = np.sum(returns * weights)
    return new[new <= VaR95].mean()


def get_portfolio_indicators(data, PF_returns, PF_prices, assets_returns, weights, rfr, config: Dict) -> Dict:
    results = {}
    port_return = Calcul_Mean(data, weights)
    ...
    port_Var_95 = Calcul_VaR_95(data, weights)
    port_CVar_95 = Calcul_CVaR_95(data, weights, port_Var_95)
    ...
    results[config['view']['Var95']] = port_Var_95
    results[config['view']['CVar95']] = port_CVar_95
